    ...
    const formDelete = (event,id,index) => {
        ...
        if(props.education.length===1){
            ...
            const updatedList = props.education.filter(
              ...
              const List = props.educationList.filter((_, index) => index !== deleteIndex);
              props.setEducationList(List)
      
              const newDegree = degree.filter((_, index) => index !== deleteIndex);
              setDegree(newDegree)
      
              const newUni = university.filter((_, index) => index !== deleteIndex);
                ...
                return(
                    ...
                    <form className="educationForm" key={education.id}>
                        ...
                        onChange={(event) => {
                            ...
                            list[index] = {degree:degree[index],university:university[index],start:start[index],end:end[index]}
                            props.setEducationList(list)
                        }}>
                        </input>
                        ...
                        onClick={(event) => formDelete(event,education.id,index)}>
                        Delete
                        </button>
